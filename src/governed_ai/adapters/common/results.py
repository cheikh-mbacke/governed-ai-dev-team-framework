"""RuntimeResult persistence under ``.ai-team/runtime-results/``, shared by every Adaptateur.

Extracted from ``adapters/cursor/runtime/results.py``. The only Cursor-specific
detail in that file was the hardcoded ``ADAPTER_ID``/``ADAPTER_VERSION`` used to
stamp ``RuntimeResult.adapter`` and to validate it back — here both become
explicit parameters (``adapter_id``/``adapter_version`` on
``build_runtime_result``, optional ``expected_adapter_id`` on
``validate_runtime_result``) so this module has no tool-specific content at all.
"""
...
def validate_runtime_result(
    ...
    *,
    ...
    if not isinstance(adapter, dict):
        ...
        if expected_adapter_id is not None and adapter.get("id") != expected_adapter_id:
            errors.append(f"adapter.id must be {expected_adapter_id}")
...
def persist_runtime_result(
    ...
    *,
    ...
    errors = validate_runtime_result(payload, expected_adapter_id=expected_adapter_id)
    if errors:
...
def load_runtime_result(
    ...
    *,
    ...
    errors = validate_runtime_result(data, expected_adapter_id=expected_adapter_id)
    if errors:
        raise ValueError("; ".join(errors))
