"""Claude Code Adapter SPI implementation (pilot skeleton, plan snug-knitting-catmull)."""
...
from adapters.claude_code.runtime.guard import (
...
# Framework source: <repo>/src/governed_ai/adapters/claude_code → parents[3] == <repo>
_SOURCE_MANIFEST = _ADAPTER_DIR.parents[3] / "adapters" / "claude_code" / "manifest.json"
_MANIFEST_PATH = (
    _COLOCATED_MANIFEST if _COLOCATED_MANIFEST.is_file() else _SOURCE_MANIFEST
)


class ClaudeCodeAdapter(AdapterSPIBase):
    """Claude Code bundle compiler and (stub) runtime harness — pilot skeleton."""
