...
def metadata() -> dict:
    ...
    framework = json.loads(
        (AI / "framework-version.json").read_text(encoding="utf-8")
    )
...
def validate_payload(payload: dict, schema_name: str) -> None:
    schema = json.loads((AI / "schemas" / schema_name).read_text(encoding="utf-8"))
    errors = sorted(
...
def find_work_unit(work_unit_id: str) -> tuple[Path, dict] | None:
    directory = AI / "work-units"
