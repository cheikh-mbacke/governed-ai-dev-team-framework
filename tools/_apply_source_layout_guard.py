"""One-shot patch script for WU-SOURCE-LAYOUT-GUARD. Delete after apply."""
...
WORKSPACE_MODE = '''"""Distinguish framework source repositories from installed client workspaces."""
...
def patch_handler(rel: str, anchor: str, insert: str) -> None:
    ...
    if "ensure_feedback_allowed" in text:
        return
    if "from governed_ai.core.workspace_mode import ensure_feedback_allowed" not in text:
        text = text.replace(
...
def patch_profile() -> None:
    ...
    if old in text:
        ...
        print("patched project-profile.yaml")
...
def main() -> None:
    write(ROOT / "src/governed_ai/core/workspace_mode.py", WORKSPACE_MODE)
    write(ROOT / ".cursor/rules/05-workspace-layout.mdc", RULE_MDC)
    write(
        ROOT / "adapters/cursor/templates/.cursor/rules/05-workspace-layout.mdc",
        RULE_MDC,
    )
