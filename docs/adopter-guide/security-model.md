...
- Remontée feedback : installer/utiliser le framework vaut acceptation (ADR-009). Export/submit full vers `https://feedback.agenteam.fr` avec HMAC par installation (`.ai-team/secrets/feedback-ingest.json`). Seul `telemetry.collection: disabled` coupe la remontée.
