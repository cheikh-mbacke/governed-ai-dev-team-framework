...
| `.cursor/skills/frontend-design/`, `.cursor/skills/webapp-testing/` | Vendored, unmodified except a setup note in webapp-testing, from https://github.com/anthropics/skills (Apache License 2.0, see each folder's NOTICE.md and LICENSE.txt). Not from the source document — added because a versioned usability policy alone doesn't produce visual taste or real evidence for it; these give frontend-developer/qa-test/code-reviewer a concrete way to produce and check that evidence rather than trusting a claim |
