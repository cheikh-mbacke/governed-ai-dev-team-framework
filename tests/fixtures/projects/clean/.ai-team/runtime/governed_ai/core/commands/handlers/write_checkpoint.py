"""WriteCheckpoint command handler — fencing-checked resumption record (Document 6 §9.2, §9.4)."""
...
def handle_write_checkpoint(
    envelope: dict[str, Any],
    *,
    workspace_root,
    transaction: Transaction,
) -> tuple[dict[str, Any], list[str]]:
    work_unit_id = envelope["target"]["id"]
    ...
    checkpoint_path = workspace_root.ai_team / "runs" / "checkpoints" / f"{work_unit_id}.yaml"
    ...
    document = {
        ...
        "last_validated_workflow_state": payload.get("last_validated_workflow_state"),
    ...
    transaction.plan_yaml_write(checkpoint_path, document)

    return {
        "affected": [
            {"kind": "checkpoint", "id": work_unit_id, "epoch": epoch},
        ],
    }, []
