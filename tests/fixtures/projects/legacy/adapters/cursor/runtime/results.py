"""RuntimeResult persistence under ``.ai-team/runtime-results/`` — Cursor binding.

Thin wrapper over ``governed_ai.adapters.common.results``, the shared
tool-agnostic implementation, binding it to Cursor's own ``ADAPTER_ID``/
``ADAPTER_VERSION`` so existing call sites (``adapters.cursor.runtime.*``,
``tests/adapters/cursor/*``, ``tests/adapters/conformance_suite.py``) keep
working unchanged.
"""
