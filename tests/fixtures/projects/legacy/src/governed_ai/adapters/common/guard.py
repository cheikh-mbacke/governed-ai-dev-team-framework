"""Pre-execution authority guards shared by every Adaptateur runtime (AD-006-010).

Extracted verbatim from ``adapters/cursor/runtime/guard.py`` — this module
operates only on ``RoleDefinitionRevision`` and the Core command vocabulary,
with no Cursor-specific content, so every Adaptateur runtime harness reuses it
directly instead of maintaining a parallel copy of authority-boundary logic.
"""
