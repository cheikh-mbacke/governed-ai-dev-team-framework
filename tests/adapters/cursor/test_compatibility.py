"""Cursor compatibility negotiation — platform normalization."""
...
MANIFEST = json.loads(
    (REPO_ROOT / "adapters" / "cursor" / "manifest.json").read_text(encoding="utf-8")
)
...
def test_normalize_platform_maps_wsl_to_linux() -> None:
    assert _normalize_platform("wsl") == ("linux", False)


def test_normalize_platform_keeps_windows_native_distinct() -> None:
    assert _normalize_platform("windows-native") == ("windows", True)
    assert _normalize_platform("windows") == ("windows", False)
    assert _normalize_platform("linux") == ("linux", False)
