...
def _transform_file(
    ...
    if not rel_posix.startswith(f"{CLAUDE_SUBDIR}/agents/") or not rel_posix.endswith(".md"):
        return content
...
def compile_manifest(
    ...
    *,
    ...
    """Validate bundle, write staged ``.claude/`` tree, return Artifact Manifest."""
    _ = project_profile  # reserved for profile-driven allowlist diffs (later increment)

    bundle_dir = bundle_dir.resolve()
    staging_dir = staging_dir.resolve()
    templates_root = (templates_root or _default_templates_root()).resolve()
    template_claude = templates_root / CLAUDE_SUBDIR
    if not template_claude.is_dir():
        raise FileNotFoundError(f"claude-code templates missing: {template_claude}")
    ...
    for src_path in sorted(template_claude.rglob("*")):
        ...
        rel_under_claude = src_path.relative_to(template_claude).as_posix()
