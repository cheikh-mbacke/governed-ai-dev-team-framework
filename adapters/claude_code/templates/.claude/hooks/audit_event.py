...
def _safe_event(payload: dict[str, Any], root: Path) -> dict[str, Any]:
    ...
    for key in ("duration", "duration_ms", "exit_code", "status", "model", "sandbox"):
...
def _prune_rotated_logs(log_dir: Path, retention_days: int) -> None:
    cutoff = datetime.now(timezone.utc).timestamp() - retention_days * 86400
    for path in log_dir.glob("claude-code-events.jsonl.*"):
...
root = (
    ...
    if env_root
    else Path(str(payload.get("cwd") or ".")).resolve()
)
...
record: dict[str, Any] = {
    ...
    "event_id": f"CLAUDE-CODE-{uuid.uuid4().hex}",
    "timestamp": datetime.now(timezone.utc).isoformat(),
    "adapter": {"id": "claude-code", "version": os.environ.get("CLAUDE_CODE_VERSION")},
    "context": {
        key: value
        for key, value in {
...
try:
    ...
    log_path = log_dir / "claude-code-events.jsonl"
